...
export default function PlannerLogin() {
  const { session, authState } = usePlannerSession();
  ...
  useEffect(() => {
    if (session && authState.state === "ADMIN") navigateToPlanner("/planner");
    if (session && authState.state !== "ADMIN") {
      setError(
        "This account is signed in but is not authorized for the planner console. An administrator must assign app_metadata.role=admin.",
      );
    }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (data.session?.user.app_metadata?.role !== "admin") {
      setError(
        "Credentials accepted, but this account is not authorized for the planner console. An administrator must assign app_metadata.role=admin.",
      );
    }
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 py-8">
      <section className="w-full max-w-md rounded-2xl bg-card p-8 shadow-2xl">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Sign in with your authorized planner account.
        </p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <label className="block text-sm font-medium text-foreground">
            ...
            <input
              ...
              className="mt-2 block w-full rounded-lg border border-input px-3 py-2.5 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          </label>
          <label className="block text-sm font-medium text-foreground">
            Password
