...
export default function AdminIssueDetails({ issue }: { issue?: Issue }) {
  ...
  if (!issue) {
    return <div className="rounded-xl border border-dashed border-input bg-card p-10 text-center"><p className="font-medium text-foreground">Issue not found</p><button type="button" onClick={() => navigateToPlanner('/planner/issues')} className="mt-4 rounded-lg border border-input px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">Back to Issues</button></div>;
  }

  return (
    <div className="space-y-6">
      <button type="button" onClick={() => navigateToPlanner('/planner/issues')} className="text-sm font-medium text-primary hover:text-primary">← Back to Issues</button>
      <section className="rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="font-mono text-xs text-muted-foreground">{issue.id}</p><h2 className="mt-2 max-w-3xl text-2xl font-bold text-foreground">{issue.description}</h2></div>
          <StatusBadge status={status} />
        </div>
        <div className="mt-6 grid gap-4 border-t border-border pt-5 sm:grid-cols-2 lg:grid-cols-4">
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Category</p><p className="mt-1 font-medium text-foreground">{CATEGORY_LABELS[issue.category]}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Severity</p><p className="mt-1 font-medium text-muted-foreground">Not tracked</p></div>
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Date reported</p><p className="mt-1 font-medium text-foreground">{new Date(issue.created_at).toLocaleString()}</p></div>
          <div><p className="text-xs uppercase tracking-wide text-muted-foreground">Last updated</p><p className="mt-1 font-medium text-muted-foreground">Not tracked</p></div>
        </div>
        {issue.sub_detail && <p className="mt-5 rounded-lg bg-muted p-3 text-sm text-foreground">{issue.sub_detail}</p>}
      </section>
      ...
      {issue.reporter_name || issue.reporter_email ? <section className="rounded-xl border border-border bg-card p-6 shadow-sm"><h3 className="text-lg font-semibold text-foreground">Reporter information</h3>{issue.reporter_name && <p className="mt-3 text-sm text-foreground">Name: {issue.reporter_name}</p>}{issue.reporter_email && <p className="mt-1 text-sm text-foreground">Email: {issue.reporter_email}</p>}</section> : null}
      </div>
  );
}
