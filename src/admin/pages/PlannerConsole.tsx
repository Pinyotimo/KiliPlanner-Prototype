...
function routeTitle(pathname: string): string {
  ...
  if (pathname === "/planner/notifications") return "Notifications";
...
function AuthenticatedPlannerConsole({ pathname }: { pathname: string }) {
  ...
  const effectivePathname =
    pathname === "/planner" && settings.defaultMapView === "map"
      ? "/planner/map"
      : pathname;
  const {
    allIssues,
    ...
    refreshing,
    refresh,
  } = useIssues({ realtimeEnabled: settings.refreshBehavior === "realtime" });
  ...
  useEffect(() => {
    const storedIds = new Set(getPlannerNotificationIds());
    if (storedIds.size === 0) return;
    setNewIssueNotifications(
      allIssues.filter((issue) => storedIds.has(issue.id)),
    );
  }, [allIssues]);
  ...
  useEffect(() => {
    ...
    setNewIssueNotifications((current) => {
      const next = current.some((issue) => issue.id === newIssue.id)
        ? current
        : [newIssue, ...current].slice(0, 10);
      setPlannerNotificationIds(next.map((issue) => issue.id));
      return next;
    });
  ...
  function markNotificationRead(issueId: string) {
    setNewIssueNotifications((current) => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("planner-notification", handleNotification);
  }, []);

  const issueId = pathname.startsWith("/planner/issues/")
  ...
  if (!loading && !error) {
    if (effectivePathname === "/planner/map")
      page = <AdminMap issues={allIssues} />;
    else if (effectivePathname === "/planner/issues")
      page = (
        <AdminIssues searchTerm={searchTerm} refreshKey={realtimeVersion} />
      );
    else if (effectivePathname.startsWith("/planner/issues/"))
      page = (
        <AdminIssueDetails
          issue={allIssues.find((issue) => issue.id === issueId)}
        />
      );
    else if (effectivePathname === "/planner/analytics")
      page = <AdminAnalytics issues={allIssues} />;
    else if (effectivePathname === "/planner/notifications")
      page = (
        <AdminNotifications
          issues={allIssues}
          unreadIssues={newIssueNotifications}
          onMarkAllRead={markNotificationsRead}
          onMarkRead={markNotificationRead}
        />
      );
    else if (effectivePathname === "/planner/reports")
      page = <AdminReports issues={allIssues} />;
    else if (effectivePathname === "/planner/settings")
      page = <AdminSettings />;
  }
  ...
  return (
    <>
      <AdminLayout
        title={
          effectivePathname === "/planner"
            ? "Dashboard"
            : effectivePathname === "/planner/map"
              ? "Live Map"
              : effectivePathname === "/planner/issues"
                ? "Issues"
                : effectivePathname === "/planner/analytics"
                  ? "Analytics"
                  : effectivePathname === "/planner/notifications"
                    ? "Notifications"
                    : effectivePathname === "/planner/reports"
                      ? "Reports"
                      : effectivePathname === "/planner/settings"
                        ? "Settings"
                        : routeTitle(effectivePathname)
        }
        showBreadcrumb={
          effectivePathname !== "/planner" &&
          effectivePathname !== "/planner/map" &&
        ...
        showSearch={effectivePathname === "/planner/issues"}
        searchValue={searchTerm}
        onSearchChange={setSearchTerm}
        refreshing={refreshing}
        onRefresh={handleRefresh}
        notifications={newIssueNotifications}
